#!/bin/sh
# displayswitcher installer — fetches the latest prebuilt arm64 binary from
...
# --- Platform check --------------------------------------------------------
[ "$(uname -s)" = "Darwin" ] || err "displayswitcher requires macOS."
ARCH="$(uname -m)"
...
PKG="displayswitcher-${TAG}-macos-arm64"
...
ok "displayswitcher ${TAG} installed."
...
echo
printf '%sTry it:%s\n' "$c_dim" "$c_reset"
echo "  displayswitcher --list"
echo "  displayswitcher --set-brightness 60 --on-display 1"
echo "  displayswitcher --help"
